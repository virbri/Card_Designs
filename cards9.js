...
function drawFrame() {
    ...
    // check if the canvas is clickable
    if (canvas.style.pointerEvents === "auto") {
      // play the animation if the canvas is clickable
      frame = spriteSheet.frames[currentFrame].frame;
      currentFrame++;
  
      // check if the current frame is beyond the end of the animation
      if (currentFrame >= frameCount) {
        currentFrame = 0;
      }
  
      // perform animation based on current frame
